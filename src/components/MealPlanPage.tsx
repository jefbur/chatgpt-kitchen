...
import { Calendar, ChefHat, Copy, Trash2, CheckCircle, History } from "lucide-react";
...
export const MealPlanPage = () => {
  ...
  const saveToHistory = () => {
    ...
    toast({
      title: "Success",
      description: "Meal plan added to history"
    });
  };
    ...
    try {
      ...
      for (const ingredient of ingredients || []) {
        ...
        if (pantryItems && pantryItems.length > 0) {
          ...
          // If quantity goes to 0, add to shopping list
          if (newQuantity === 0) {
      ...
      toast({
        title: "Success",
        description: "Meal marked as cooked and ingredients deducted"
      });
    } catch (error) {
  ...
  const clearWeekPlan = async () => {
    if (!confirm('Are you sure you want to clear this week\'s meal plan?')) return;
  ...
  const generateGroceryList = async () => {
    try {
      ...
      // Check against current pantry and add to shopping list
  ...
  if (loading) {
    return <div className="text-center p-8">Loading meal plans...</div>;
  }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Meal Planning in Jackson
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Tabs value={currentWeek === 'A' ? 'A' : currentWeek === 'B' ? 'B' : 'history'} onValueChange={(value) => {
            if (value === 'A' || value === 'B') {
              setCurrentWeek(value as 'A' | 'B');
            }
          }}>
            ...
            <TabsContent value={currentWeek} className="space-y-4">
              ...
              <Card>
                <CardContent className="p-4">
                  <Textarea
                    placeholder="Week notes and reminders..."
                    value={weekNotes}
              ...
              <div className="flex flex-wrap gap-2 pt-4 border-t">
                <Button variant="lavender" onClick={generateGroceryList}>
            ...
            <TabsContent value="history" className="space-y-4">
              <div className="text-center text-muted-foreground mb-4">
                ...
                <p className="text-sm">Previous meal plans for reference</p>
              </div>
                ...
                <Card className="text-center p-8">
                  <CardContent>
                    <p className="text-muted-foreground">
                      No meal plan history yet. Save your current meal plan to start building history.
                    </p>
                  </CardContent>
                </Card>
              ) : (
                    ...
                    <Card key={index} className="border-2">
                      <CardHeader className="pb-3">
                        <CardTitle className="text-lg flex items-center gap-2">
                          ...
                          {entry.date} - Week {entry.weekType}
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="overflow-x-auto">
                          <div className="min-w-[800px]">
                            ...
                            {/* Meal rows */}
                            {mealTypes.map(mealType => (
                              <div key={mealType.name} className="grid grid-cols-8 gap-2 mb-2">
                                <div className="font-medium text-sm">{mealType.name}</div>
                                {daysOfWeek.map(day => (
                                  <div key={day} className="text-xs">
                                    {entry.meals[day]?.[mealType.name]?.join(', ') || ''}
                                  </div>
                                ))}
                              </div>
                            ))}
